...
function onEffectTick(target,effect)
    ...
    if (Half_Minutes == 1) then
        ...
        target:messageSpecial(text.MARK_OF_SEED_FLICKERS);
    elseif (Half_Minutes == 40) then
        target:messageSpecial(text.MARK_OF_SEED_IS_ABOUT_TO_DISSIPATE);
        target:setVar("SEED_AFTERGLOW_TIMER",(Half_Minutes+1));
    elseif (Half_Minutes == 20) then
        target:messageSpecial(text.MARK_OF_SEED_GROWS_FAINTER);
        target:setVar("SEED_AFTERGLOW_TIMER",(Half_Minutes+1));
...
function onEffectLose(target,effect)
    if (target:hasKeyItem(MARK_OF_SEED) == false and player:hasKeyItem(AZURE_KEY) == false) then
        target:messageSpecial(text.MARK_OF_SEED_HAS_VANISHED);
    end
